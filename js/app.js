...
window.previewDietVisual = (diet) => {
    ...
    const renderOptions = (opts) => opts.map((o,i) => `
        <div class="option-card">
            <div style="color:var(--text-muted); font-size:0.8rem; font-weight:800; margin-bottom:5px;">OPCIÓN ${String.fromCharCode(65+i)}</div>
            <div style="color:#eee; line-height:1.4;">${o.desc}</div>
        </div>`).join('');
    
    const header = (ico, tit, k) => `<div class="meal-title"><span style="display:flex; align-items:center; gap:10px;"><i class="${ico}"></i> ${tit}</span> <span style="font-weight:normal; color:#888;">${diet.isAdLibitum?'':k+' kcal'}</span></div>`;
    ...
    container.innerHTML = `
        <div class="diet-hero">
            <div>
                ...
                <h1 style="color:white; margin-bottom:10px; font-size:1.8rem;">${diet.name}</h1>
            ...
            <div class="card" style="align-items:center; justify-content:center;">
                <h5 style="color:#888; margin-bottom:15px;">DISTRIBUCIÓN MACROS</h5>
                <div style="height:180px; width:180px; position:relative;"><canvas id="${chartId}"></canvas></div>
            </div>
        </div>

        ${mealsHtml}

        <h2 style="margin-top:40px; margin-bottom:20px; border-top:1px solid #333; padding-top:20px; color:white;">
            <i class="bi bi-journal-check"></i> Guía & Reglas
        </h2>
        ...
        <div class="warning-box">
            ...
            <ul style="padding-left:20px; margin-top:10px; color:#ffcc80;">
                ...
                <li>El aceite cuenta: ¡Mídelo!</li>
            </ul>
        </div>

        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:20px; margin-top:20px;">
            <div class="card" style="background:rgba(76, 175, 80, 0.1); border-color:rgba(76, 175, 80, 0.3);">
                <h4 style="color:#66bb6a; margin:0 0 10px 0;"><i class="bi bi-check-circle"></i> PERMITIDO</h4>
                <ul style="padding-left:20px; color:#ddd;">${guide.allowed.map(i=>`<li>${i}</li>`).join('')}</ul>
            </div>
            <div class="card" style="background:rgba(244, 67, 54, 0.1); border-color:rgba(244, 67, 54, 0.3);">
...
function renderEducation(){ const c=document.getElementById('eduContent'); if(!userData.currentDietData){c.innerHTML='Sin plan';return;} const guide=dietGuides[userData.currentDietData.category]; c.innerHTML=`<div class="warning-box"><h3>Tips ${userData.currentDietData.category}</h3><ul>${guide.tips.map(t=>`<li>${t}</li>`).join('')}</ul></div>`; }
