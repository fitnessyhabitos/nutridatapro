...
window.navigate = (sid) => {
    ...
    const view = isAdmin ? 'adminView' : 'athleteView'; document.getElementById(view).style.display='block';
    ['clients','diets','inbox', 'myPlan','evolution','education','history','notes'].forEach(x => { const el = document.getElementById(x+'-section'); if(el) el.style.display = (x===sid) ? 'block' : 'none'; });
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        currentUser = user; isAdmin = (user.email.toLowerCase().trim() === ADMIN_EMAIL.toLowerCase().trim());
        document.getElementById('landingSection').style.display = 'none'; document.getElementById('appLayout').style.display = 'block';
        if (isAdmin) initAdminDashboard(); else { const snap = await getDoc(doc(db,"clientes",user.uid)); if(snap.exists()){ userData=snap.data(); initAthleteDashboard(); } else if(isAdmin) initAdminDashboard(); }
    } else {
        currentUser=null; isAdmin=false; document.getElementById('landingSection').style.display = 'flex'; document.getElementById('appLayout').style.display = 'none'; document.getElementById('boxLogin').style.display = 'block'; document.getElementById('boxRegister').style.display = 'none';
    }
});

function renderMenus(items) {
    const d=document.getElementById('desktopMenu'), m=document.getElementById('mobileMenu'); d.innerHTML=''; m.innerHTML='';
    items.forEach(i => {
