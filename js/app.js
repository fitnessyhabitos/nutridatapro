...
function initAdminDashboard() {
    ...
    // Inyectar Select Filtro si no existe
    if(!document.getElementById('filterCategory')){
        const sel = document.createElement('select');
        sel.id = 'filterCategory';
        sel.className = 'glass-input';
        sel.style.width = 'auto'; sel.style.margin = '0';
        ...
        sel.onchange = filterDiets;
        // Insertar antes del buscador
        tb.insertBefore(sel, document.getElementById('searchKcal').parentNode);
    }
}
    ...
    container.innerHTML = `
        <div class="diet-hero">
            ...
            <div class="card" style="align-items:center; justify-content:center;">
                <h5 style="color:#888; margin-bottom:15px;">MACROS</h5>
        ...
        <div class="warning-box">
            ...
            <ul style="padding-left:20px; margin-top:10px; color:#ffcc80;">
                ...
                <li>Pesar todo en CRUDO (incluido verduras y aceite).</li>
            </ul>
        </div>
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:20px; margin-top:20px;">
            <div class="card" style="background:rgba(76, 175, 80, 0.1); border-color:rgba(76, 175, 80, 0.3);">
                <h4 style="color:#66bb6a; margin:0 0 10px 0;">PRIORIZAR</h4>
            ...
            <div class="card" style="background:rgba(244, 67, 54, 0.1); border-color:rgba(244, 67, 54, 0.3);">
                <h4 style="color:#ef5350; margin:0 0 10px 0;">EVITAR</h4>
...
window.resetDatabaseManual = async () => {
    ...
    if(!confirm("⚠️ Se borrarán TODAS las dietas y se crearán las nuevas (+150).")) return;
...
window.filterDiets = () => {
    ...
    const filtered = window.allDietsCache.filter(d => {
        const matchText = d.name.toLowerCase().includes(term) || d.calories.toString().includes(term);
